...
// RTL patch: replace slide-in-from-right/left with RTL-aware directions
  ...
  'fixed z-50 gap-4 bg-[var(--surface)] p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      side: {
        ...
        // RTL patch: 'left' in RTL means visual end (right side physically)
        left: 'inset-y-0 start-0 h-full w-3/4 border-e border-[var(--border)] data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm',
        // RTL patch: 'right' in RTL means visual start (left side physically)
        right:
          'inset-y-0 end-0 h-full w-3/4 border-s border-[var(--border)] data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm',
      },
    },
    defaultVariants: {
      side: 'bottom',
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('flex flex-col-reverse sm:flex-row sm:justify-end sm:space-s-2', className)}
    {...props}
  />
)

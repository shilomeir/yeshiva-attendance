...
import { Clock, StopCircle, Tv } from 'lucide-react'
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--ink-faint)', marginTop: 4, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <Clock size={12} />
          זמן מתחילת ביקורת: <span style={{ fontFamily: 'JetBrains Mono, monospace', color: 'var(--ink-muted)' }}>{elapsed}</span>
          {session.mode === 'LOCATION' ? ' • מצב: עם מיקום' : ' • מצב: ידנית'}
        </div>
      </div>

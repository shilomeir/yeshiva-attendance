-- Two overloaded versions of submit_departure existed in the DB, causing PostgREST
-- to fail with "not found in schema cache" on every departure submission.
-- Fix: drop both overloads and recreate as a single 11-param function where
-- p_actor_pin is accepted (DEFAULT NULL) but ignored — PIN auth is client-side.
-- This lets both the old frontend (which sent p_actor_pin) and the new frontend
-- (which doesn't) call the same function without any schema cache ambiguity.
